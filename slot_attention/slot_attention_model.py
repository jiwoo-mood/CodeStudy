...
class SlotAttention(layers.Layer):
  ...
  def __init__(self, num_iterations, num_slots, slot_size, mlp_hidden_size,
    ...
    self.project_v = layers.Dense(self.slot_size, use_bias=False, name="v") #v는 어캐되는지?
  ...
  def call(self, inputs):
    ...
    v = self.project_v(inputs)  # Shape: [batch_size, num_inputs, slot_size]. #v == inputs

    # Initialize the slots. Shape: [batch_size, num_slots, slot_size].
    #slot 초기화 할 때 가우시안 분포에서 샘플링을 함. 이때 가우시안의 mu, sigma가 learnable 
