...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ```
"""
#Samplig하기 전에 g 커널 적용
def gaussian_kernel(kernel_size=3, sigma=1.0, channels=3):
    x_coord = torch.arange(kernel_size)
    #논문에서는 sigma 1에서 0.01로 줄어들게끔 함.
    #a3=1이라서 코드에 없는듯?
...
class DemoFusionSDXLPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin, TextualInversionLoaderMixin):
    ...
    def encode_prompt( #enocode되는 prompt의 inputs / demo.py에 값이 적혀있음. / 적혀있지 않은 경우 각각 변수를 담당하는 code가 생성해주거나 pass함.
        self,
        prompt: str,
    ...
    # Copied from diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion.StableDiffusionPipeline.prepare_latents 잠재 변수로 변환?하는 함수
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None):
        ...
        if latents is None: #latents가 제공되지 않으면 무작위로 노이즈 텐서를 생성함.
            latents = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
        else:
            latents = latents.to(device)

        # scale the initial noise by the standard deviation required by the scheduler
        latents = latents * self.scheduler.init_noise_sigma #scheduler가 sigma를 조절함.
        return latents
    ...
    def get_views(self, height, width, window_size=128, stride=64, random_jitter=False): #Fi는 mappings defining crops from the panorama
        # Here, we define the mappings F_i (see Eq. 7 in the MultiDiffusion paper https://arxiv.org/abs/2302.08113)
        # if panorama's height/width < window_size, num_blocks of height/width should return 1 파노라마 하면 widow size보다 클텐데 작다면, 1로 리턴시킴.
        
        #self.vae_scale_factor = 2 ** (len(self.vae.config.block_out_channels) - 1)
        #파노라마 이미지를 VAE에 따라 resize하기 위해 h,w를 resize
        height //= self.vae_scale_factor
        width //= self.vae_scale_factor

        #파노라마를 만들고자 하고 window 크기마다 block를 자름. 그때마다의 view, stride는 view 사이의 간격?
        ...
        for i in range(total_num_blocks):
            h_start = int((i // num_blocks_width) * stride) #이런식으로 h,w 를 만드네 stride, window size 간격으로 block 만드는듯 (block 생성 및 위치 조정)
            h_end = h_start + window_size
            w_start = int((i % num_blocks_width) * stride)
            w_end = w_start + window_size

            if h_end > height: #다음 블록인 경우 
            ...
            if random_jitter: #block 랜덤으로 위치를 변경하는 augmentation??
                jitter_range = (window_size - stride) // 4 #일정 범위 내로 랜덤으로 조정
                w_jitter = 0
                h_jitter = 0
            ...
            views.append((h_start, h_end, w_start, w_end)) #파노라마 이미지 내 window size로 잘랐을때의 블록에서의 view
        return views

    def tiled_decode(self, latents, current_height, current_width): #latent를 decoding (image를 block(tile)로 나눴어서 그것에 대해 여러번 decoding을 할 듯.)
        ...
        with self.progress_bar(total=len(views_batch)) as progress_bar:
            for j, batch_view in enumerate(views_batch):
                ...
                #VA3를 이용해 latents를 디코딩하고 image_patch 생성
                ...
                #생성되고, scaling 된 이미지를 image 텐서에 넣음.
                image[:, :, h_start:h_end, w_start:w_end] += image_patch[:, :, p_h_start:p_h_end, p_w_start:p_w_end].to(latents.device)
                count[:, :, h_start:h_end, w_start:w_end] += 1
                progress_bar.update()

        #특정 위치에 대해 여러 번 image patch가 덮어졌다면 그 위치는 평균값을 갖도록 함. (겹치거나 경계에서 여러 번 계산될 수 있기 때문에 그것을 보완하고자 함.)
        image = image / count
    ...
    @torch.no_grad()
    @replace_example_docstring(EXAMPLE_DOC_STRING) #evaluation
    def __call__(
        self,
        ...
        num_inference_steps: int = 50, #DDIM scheduler 50 steps
        ...
        view_batch_size: int = 16, #The batch size for multiple denoising paths
        multi_decoder: bool = True, #이미지의 resolution이 3072x3072 넘으면, a tiled decoder 필수 나눠서 디코딩하라는 거임.
        stride: Optional[int] = 64, #The stride of moving local patches.
        cosine_scale_1: Optional[float] = 3., #Skip Residual 용
        cosine_scale_2: Optional[float] = 1., #Sampling 용
        cosine_scale_3: Optional[float] = 1., #gaussian filter의 표준편차 용
        sigma: Optional[float] = 1.0, #1에서 0.01로 줄여가면서 가우시안 필터 적용
        show_image: bool = False,
        lowvram: bool = False,
        stage1_resolution: Optional[int] = None,
        jump_step: int = 1,
        use_DS: Optional[bool] = True,
    ):
        r"""
        ...
        if stage1_resolution != None:
            self.unet.config.sample_size = stage1_resolution // self.vae_scale_factor
            self.default_sample_size = self.unet.config.sample_size

        height = height or self.default_sample_size * self.vae_scale_factor # target height
        width = width or self.default_sample_size * self.vae_scale_factor # target width

        x1_size = self.default_sample_size * self.vae_scale_factor # 1024x1024 (default = latent size = 1024 (sdxl), vae scale factor = 8)

        height_scale = height / x1_size # x1 대비 target resolution의 배수 --> upsampling
        width_scale = width / x1_size # 배수
        scale_num = int(max(height_scale, width_scale)) # 최종 배수/ 몇배 늘릴지? 를 size 등으로 계산해서 max 값 취하는 듯.
        aspect_ratio = min(height_scale, width_scale) / max(height_scale, width_scale)

        original_size = original_size or (height, width)
        target_size = target_size or (height, width) # 왜 target_size랑 height 이 따로 있지..? 헷갈리게 생겼는데. README 보면 실제로도 target_size를 따로 사용하진 않는듯
        ...
        if prompt is not None and isinstance(prompt, str): #batch size 정함.
            batch_size = 1
        elif prompt is not None and isinstance(prompt, list):
            batch_size = len(prompt)
        else:
        ...
        latents = self.prepare_latents( #VAE의 이미지 생성 과정에서 latent variables를 준비하는 역할.
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height // scale_num, # height // 최종 배수. -> 만약에 h, w 의 배수가 다르면.. 그만한 크기가 생성 안될텐데 어떡하려고..?
            width // scale_num, #배수를 동일하게 받나?
            prompt_embeds.dtype,
            device,
            generator,
            latents,
        ...
        # 7. Prepare added time ids & embeddings 프롬프트와 관련된 embedding.
    ...
    ###################################################### Phase Initialization ########################################################
        ...
        #Random Initalization 언제 했는지... 다시 살펴보기
        #paper의 Appendix C
        if image_lr == None: # Low-resolution image input for upscaling. If provided, DemoFusion will encode it as the initial latent representation.
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    # perform guidance
                    if do_classifier_free_guidance: #CFG 이용해서 guidance로 scaling함. --> 생성할 때 conditioning 방향으로 학습되게끔 도와주는 역할. hyperparameter임.
                        noise_pred_uncond, noise_pred_text = noise_pred[::2], noise_pred[1::2]
                        noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond) #CFG 공식
        ...
        if not output_type == "latent": #latent vector를 디코딩하겠다는 의미. output_type이 latent가 아니므로 디코딩하여 outputs를 image로 전환
        ...
        if show_image:
            ...
            plt.show()
        output_images.append(image[0]) #첫번째 이미지
                        
    ####################################################### Phase Upscaling #####################################################
        TIMECOST = []
        if image_lr == None: # 1024 resolution
            starting_scale = 2
        else: # 512 resolution
            starting_scale = 1
        #PU를 없앨거면, jump_step이 scale ratio-1이어야 함.
        for current_scale_num in range(starting_scale, scale_num + 2, jump_step): #scale 하는 횟수. (3번할 거면 3번 upsampling 해야하므로. 반복문 안에서 계속 diffusion pass돌아감.) jump_step하면 일정 단계를 넘김.
            if self.lowvram:
                latents = latents.to(device)
                self.unet.to(device)
                torch.cuda.empty_cache()
            ...
            #latent space에서의 bicubic으로 interpolation
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    c1 = cosine_factor ** cosine_scale_1 #Skip Residual 용
                    latents = latents * (1 - c1) + noise_latents[i] * c1 # skip residual ^zt 를 이용해서 zt-1 단계를 예측  equ4 in paper
                    
                    ############################################# MultiDiffusion (Crop Sampling) #############################################
                    
                    views = self.get_views(current_height, current_width, stride=stride, window_size=self.unet.config.sample_size, random_jitter=True) #파노라마 view
                    views_batch = [views[i : i + view_batch_size] for i in range(0, len(views), view_batch_size)]

                    jitter_range = (self.unet.config.sample_size - stride) // 4
                    latents_ = F.pad(latents, (jitter_range, jitter_range, jitter_range, jitter_range), 'constant', 0) #aug 조정
                    ...
                    c2 = cosine_factor ** cosine_scale_2 #Sampling, earlier steps mainly reconstruct the overall structure, while later steps focus on refining the detail.

                    #S(Zlocal,^zst) --> Z(local, t)
                    ...
                    if use_DS:
                        ...
                        #gaussian filter 를 먼저 해주고 Dilated sampling 한다. dilated sampling이 grainy image를 만들게 하는 경향(bc. local path랑 다르게 overlap으로 작동x) 이 있어서 이걸 해소하고자 함.
                        ...
                        for j, batch_view in enumerate(views_batch):
                            ...
                            # predict the noise residual
                            added_cond_kwargs = {"text_embeds": add_text_embeds_input, "time_ids": add_time_ids_input}
                            noise_pred = self.unet( #denoising
                                latent_model_input,
                                t,
                                encoder_hidden_states=prompt_embeds_input,
                                cross_attention_kwargs=cross_attention_kwargs,
                        ...
                        #S(Global,^zst) --> Z(Global, t)
                        value += value_global * c2
                        count += torch.ones_like(value_global) * c2
                    
                           ###########################################################   
                
                    latents = torch.where(count > 0, value / count, value) #Fusing Local and Global paths
    ...
    #########################################################################################################################################
                ...
                if not output_type == "latent":
                    ...
                    if show_image:
                        ...
                        plt.axis('off')  # Turn off axis numbers and ticks
                        plt.show()
                    output_images.append(image[0]) #progressive processing image
